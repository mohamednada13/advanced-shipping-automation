...
def send_shipping_email(excel_filepath, origin, destination):
    ...
    print("📨 Dispatching Report via Secure SMTP Module...")
    msg = MIMEMultipart()
    msg['From'] = SENDER_EMAIL
    msg['To'] = RECEIVER_EMAIL
    msg['Subject'] = f"📊 Freight Rates Report: {origin} ➡️ {destination}"
    body = f"Hi Mohamed,\n\nAttached is the automated enterprise detailed freight rates comparison report generated via decoupled multi-module architecture.\n\nRoute Details: {origin} to {destination}\n\nRegards,\nYour Advanced Cloud Automation System."
    ...
    try:
        ...
        server.quit()
        print("🚀 Success! Enterprise rate report delivered successfully.")
...
if __name__ == "__main__":
    ...
    if is_ocean:
        final_data = calculate_ocean_freight(origin_input, dest_input, weight_input, volume_input, count_20ft_input, count_40ft_input, ship_type, cargo_value_input, target_currency_input, fx_rate, currency_symbol)
    else:
        final_data = calculate_air_freight(origin_input, dest_input, weight_input, volume_input, cargo_value_input, target_currency_input, fx_rate, currency_symbol)
